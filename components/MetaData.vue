...
export default {
  ...
  computed: {
    ...
    socialTitle() {
      try {
        if (this.content.social.facebook.title)
          return this.content.social.facebook.title;
    ...
    socialDescription() {
      try {
        if (this.content.social.facebook.description)
          return this.content.social.facebook.description;
    ...
    socialImage() {
      try {
        if (this.content.social.facebook.image)
          return this.content.social.facebook.image;
        return this.fallback.image.image[0].url;
      } catch (error) {
        return "";
      }
    },
  ...
  head() {
    ...
    if (this.$get(this.content, "advanced.canonical"))
      content.link.push({
        rel: "canonical",
        href: this.content.advanced.canonical,
      });
    if (this.$get(this.content, "advanced.robots"))
      content.meta.push({
        hid: "robots",
        name: "robots",
        content: this.content.advanced.robots,
      });
